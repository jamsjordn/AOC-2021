export function part2(input: string): number {
  ...
  instructions.forEach((instruction, _instructionIndex) => {
  ...
  const dotMatrix: string[][] = [];

  Object.entries(dots).forEach(([xIndex, ys]) => {
    if (ys === undefined) { return; }

    Object.entries(ys).forEach(([yIndex, value]) => {
      if (yIndex === undefined) { return; }

      dotMatrix[+yIndex] = dotMatrix[+yIndex] || [];

      dotMatrix[+yIndex][+xIndex] = 'X';
    });
  });
